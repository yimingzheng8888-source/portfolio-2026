...
out=repo/'public/videos';inputs=['saloon_test 2.mp4','Panorama_1.mp4','stern.mp4','saloon_test 2.mp4'];cmd=[ff,'-v','warning','-y']
for f in inputs:cmd+=['-i',str(src/f)]
filters=';'.join([f'[{i}:v]trim=start={start}:duration={dur},setpts=PTS-STARTPTS,fps=30,format=yuv420p,setsar=1[v{i}]' for i,(start,dur) in enumerate([(.6,4.15),(.7,6),(.1,3.2),(0,.6)])])
filters+=';[v0][v1]xfade=transition=fade:duration=0.6:offset=3.55[x1];[x1][v2]xfade=transition=fade:duration=0.6:offset=8.95[x2];[x2][v3]xfade=transition=fade:duration=0.6:offset=11.55,format=yuv420p[out]'
...
for w in [480,960,1600,1920]:
 name=f'drift-showreel-poster-{w}.webp';im.resize((w,round(w*im.height/im.width)),Image.Resampling.LANCZOS).save(repo/'public/media'/name,quality=90,method=6);variants.append({'src':'/media/'+name,'width':w})
