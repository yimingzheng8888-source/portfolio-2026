...
export function AboutPage(){return <div className="about-page section-shell">
  ...
  <section className="about-section"><div><h2>组织与项目经历</h2><p>在协作中，把想法推进到交付。</p></div><div>{experiences.map(exp=><article className="experience-item" key={exp.id}><time>{exp.year}</time><h3>{exp.company}</h3><p className="experience-role">{exp.position}</p><p>{exp.description}</p></article>)}</div></section>
  ...
  <section className="about-section"><h2>技能与工具</h2><div className="tool-groups">{[['dev','三维与视觉表达'],['design','船舶工程工具'],['tool','分析与数据工具']].map(([category,label])=><div key={category}><h3>{label}</h3><div>{skillTags.filter(t=>t.category===category).map(t=><span key={t.id}>{t.label}</span>)}</div></div>)}</div></section>
</div>}
