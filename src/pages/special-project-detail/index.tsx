...
function Gallery({ images, title, labels = [] }: { images: string[]; title: string; labels?: string[] }) {
  ...
  return <section className="case-section"><h2>{title}</h2><p className="case-hint">点击图片查看大图；支持方向键切换与 Esc 关闭。</p>
    <div className={`case-gallery ${title === '完整竞赛展板' ? 'poster-gallery' : ''}`}>
      {images.map((src, i) => <figure key={src}><button className="gallery-trigger" onClick={e => { trigger.current = e.currentTarget; setIndex(i); }} aria-label={`放大${labels[i] || `${title} ${i + 1}`}`}><img src={url(src)} alt={labels[i] || `${title} ${i + 1}`} loading="lazy" decoding="async" /><span aria-hidden="true">放大查看</span></button>{labels[i] && <figcaption>{labels[i]}</figcaption>}</figure>)}
    </div>
...
export const SpecialProjectDetailPage: React.FC = () => {
  ...
  return <article className={`design-case ${isDrift ? 'drift-case' : ''}`} key={id}>
    ...
    <div className="case-container"><img className="case-cover" src={url(project.cover)} alt={project.title} width={1600} height={900} /></div>
    ...
    <div className="case-container">
      ...
      <section className="case-section case-story" id="project-story"><h2>{isDrift ? '一座可以航行的文化驿站' : '项目说明'}</h2><div>{Object.entries(project.sections).map(([key,value]) => value && <div key={key} className="case-paragraph"><h3>{({background:'设计背景',challenge:'设计挑战',solution:'设计策略',result:'项目成果'} as Record<string,string>)[key]}</h3><p>{value}</p></div>)}</div></section>
      ...
      {project.posters && <div id="project-posters"><Gallery images={project.posters} title="完整竞赛展板" /></div>}
